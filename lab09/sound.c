...
uint8_t volume = 5; // set to volume 5 cause we increment by 4 for the volume control
...
void writeDACs( void ) {
  ...
  if(bufferRefilled == true){
      ...
      if(dataLength > 0){
          ...
          // ADD LOTS OF YOUR CODE AROUND THESE THREE LINES
...
void upVolume( void ) {
    if(volume == 1) // volume 1 is the lowest level
        return;
    volume= volume-4; //increment the volume
}

// Future implementation: decrease volume
void downVolume( void ) {
    if(volume > 10) // volume 10 is the loudest volume level
        return;
    volume = volume +4;//decrement the volume
