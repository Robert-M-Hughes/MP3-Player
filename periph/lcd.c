...
#define assertPIN(val,pin) {if(val)GPIO_PORTF[pin] = 0; else GPIO_PORTF[pin] = pin;}
#define assertReset(val)   assertPIN(val,RST_PIN)
#define assertCS(val)      assertPIN(val,CS_PIN)
#define assertWR(val)      assertPIN(val,WR_PIN)
#define assertRD(val)      assertPIN(val,RD_PIN)
#define A0Low()            {GPIO_PORTF[A0_PIN] = 0;}
#define A0High()           {GPIO_PORTF[A0_PIN] = A0_PIN;}
#define writeDB(val)       {GPIO_PORTK[GPIO_ALLPINS] = val;}

// Write one byte to LCD.  
static void writeLCD( uint8_t c ) {
// INSERT YOUR CODE HERE.  YOU SHOULD ONLY USE THE MACROS ABOVE, AND
// ANY CODE NEEDED FOR TIMING DELAYS.

    writeDB( c );//write all the data
    assertCS( true );
    assertWR( true );//low wr
    assertWR( true );//low wr

    assertWR( false );//high wr
    assertWR( false );//high wr
...
void initLCD( void ) {
  // Enable Port F and Port K
    //sysctl to activate the pins
        SYSCTL[SYSCTL_RCGCGPIO] |= SYSCTL_RCGCGPIO_PORTF | SYSCTL_RCGCGPIO_PORTK;
        SYSCTL[SYSCTL_RCGCGPIO] |= SYSCTL_RCGCGPIO_PORTF | SYSCTL_RCGCGPIO_PORTK;


  // Set all pins on Port K to inputs initially, then enable.
        GPIO_PORTK[GPIO_DIR] &= ~GPIO_ALLPINS;
