...
uint8_t getDisplayDataSize( void ) {
  ...
  // must be calculated, not hard-coded.
    return sizeof(displayData)/sizeof(displayData[0]);
}
...
void init7SegLED( void ) {
  ...
  SYSCTL[SYSCTL_RCGCGPIO] |= SYSCTL_RCGCGPIO_PORTD | SYSCTL_RCGCGPIO_PORTD;
  SYSCTL[SYSCTL_RCGCGPIO] |= SYSCTL_RCGCGPIO_PORTE | SYSCTL_RCGCGPIO_PORTE;
...
void displayVal(uint8_t display, uint8_t value) {
  // Activate and deactivate the two transistors to turn on only one display.
    if(display == 0){
        GPIO_PORTE[GPIO_PIN_0 | GPIO_PIN_1] = GPIO_PIN_0;
    }
    else{
        GPIO_PORTE[GPIO_PIN_0 | GPIO_PIN_1] = GPIO_PIN_1;
